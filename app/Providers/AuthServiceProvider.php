...
class AuthServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        // Auth gates for: Account Receivables
        Gate::define('accounts_management', function($user){
            return in_array($user->role_id, [1,3]);
        });
        Gate::define('accounts_access', function($user){
            return in_array($user->role_id, [1,3]);
        });
        Gate::define('accounts_close', function($user){
            return in_array($user->role_id, [1,3]);
        });
        // Auth gates for: Payments
        Gate::define('payments_access', function($user){
            return in_array($user->role_id,[1,3]);
        });
